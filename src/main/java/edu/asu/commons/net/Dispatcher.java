 ...
 * <p>
 * The Dispatcher could in theory be an EventHandler/EventGenerator as well but to avoid confusion between local and remote messaging we use a transmit() method
 * to dispatch outgoing Events across the network and keep a handle on an explicit EventHandler that receives and dispatches incoming Events from across the
